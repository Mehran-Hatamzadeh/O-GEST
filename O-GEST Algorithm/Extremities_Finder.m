...
optimal_fo=[1.0707    0.2345    0.1823   -0.5032   0.2334   -0.9047];
optimal_fs=[0.4534   -0.1296    0.2767   -0.1393  -0.1165   -0.4022  0.4];
...
for i=1:1:N_Cycle_L
    ...
    %=============
    Section_Smoothed=Model_Interpolator(CPs_L,Section,INFO_L.Knots);
    INFO_L.Sections_Smoothed{1,i}=Section_Smoothed';
    Section=Section_Smoothed;
    %=============
    ...
    Section_Speed=(Section(2,2:end)-Section(2,1:end-1))./(Section(1,2:end)-Section(1,1:end-1));
    Vs=Section_Speed_Total;
    SMeanALL=mean(Section_Speed_Total);
    [~,b]=min(Section_Speed);
    tspan=(Section(1,b)-Section(1,1));
    if Indic=="FO"
        if abs(SMeanALL)>=0.8
            if tspan<=0.35
                FO_Line_Speed=optimal_fo(1,1).*(mean(Vs)) + optimal_fo(1,2).*(std(Vs));
            else
                FO_Line_Speed=optimal_fo(1,5).*(mean(Vs)) + optimal_fo(1,6).*(std(Vs));
            end
        end
        if abs(SMeanALL)<0.8
            FO_Line_Speed=optimal_fo(1,3).*(mean(Vs)) + optimal_fo(1,4).*(std(Vs));
        end
       T1= INTS_L(1,i);
       D1= INTS_L(2,i);
       T2= INTS_L(1,i+1);
    ...
    elseif Indic=="FS"
        if abs(SMeanALL)>=0.6
            FS_Line_Speed=optimal_fs(1,1).*(mean(Vs)) + optimal_fs(1,2).*(std(Vs));
        end
        if abs(SMeanALL)<0.6
            FS_Line_Speed=optimal_fs(1,3).*(mean(Vs)) + optimal_fs(1,4).*(std(Vs));
        end
        T1= INTS_L(1,i);
        D1= INTS_L(2,i);
        T2= INTS_L(1,i+1);
...
for i=1:1:N_Cycle_R
    ...
    %=============
    Section_Smoothed=Model_Interpolator(CPs_R,Section,INFO_R.Knots);
    INFO_R.Sections_Smoothed{1,i}=Section_Smoothed';
    Section=Section_Smoothed;
    %=============
    if Section(2,end)>Section(2,1)
       Section(2,:) = - Section(2,:);
...
for i=1:1:N_Cycle_L
    ...
    Section_Speed=(Section(2,2:end)-Section(2,1:end-1))./(Section(1,2:end)-Section(1,1:end-1));
    Vs=Section_Speed_Total;
    if (max(Vs)/abs(min(Vs)))<=0.1
        FS_Line_Speed=optimal_fs(1,5).*(mean(Vs)) + optimal_fs(1,6).*(std(Vs));
    end
    if (max(Vs)/abs(min(Vs)))>0.1
        FS_Line_Speed=optimal_fs(1,7).*(max(Vs));
    end
